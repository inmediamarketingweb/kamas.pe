import './Perfiles.css';

function Perfiles(){
    return(
        <section className='block-container'>
            <div className='block-content'>
                <div className='d-flex-column gap-10'>
                    <p className='title uppercase text'>Distribuidores exclusivos</p>
                    <ul className='profiles'>
                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/homesleep.webp" alt="Homesleep" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+12800 vendidos</p>
                                </div>

                                <p><a href='https://homesleep.pe/' title='Homesleep es distribuidor exclusivo autorizado kamas'>Homesleep</a> es distribuidor exclusivo autorizado kamas</p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/vitai.webp" alt="Vitai" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+10500 vendidos</p>
                                </div>

                                <p><a href='https://vitai.pe/' title='Vitai es distribuidor exclusivo autorizado kamas'>Vitai</a> es distribuidor exclusivo autorizado kamas</p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/dormihogar.webp" alt="Dormihogar" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+15000 vendidos</p>
                                </div>

                                <p><a href='https://dormihogar.pe/' title='Dormihogar es distribuidor exclusivo autorizado kamas'>Dormihogar</a> es distribuidor exclusivo autorizado kamas</p>
                            </div>
                        </li>
                    </ul>
                </div>

                <div className='d-flex-column gap-10'>
                    <p className='title uppercase text'>Marketplaces y retailers</p>
                    <ul className='profiles'>
                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/falabella.webp" alt="Falabella" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+4200 vendidos</p>
                                </div>

                                <p>Encuentra nuestros productos en <a href='https://www.falabella.com.pe/falabella-pe/search?Ntt=kamas' title='Encuentra nuestros productos en Falabella'>Falabella</a></p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/ripley.webp" alt="Ripley" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+2900 vendidos</p>
                                </div>

                                <p>Encuentra nuestros productos en <a href='https://simple.ripley.com.pe/tienda/kamas-6051854?srsltid=AfmBOopL84VUl4V0ciZz27aVgPH7wR2DhqLK5CE1-EEXA6VwJPYPDAqg&type=catalog' title='Encuentra nuestros productos en Ripley'>Ripley</a></p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/plaza-vea.webp" alt="Plaza Vea" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+2500 vendidos</p>
                                </div>

                                <p>Encuentra nuestros productos en <a href='/' title='Encuentra nuestros productos en Plaza Vea'>Plaza vea</a></p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/juntoz.webp" alt="Juntoz" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="Kamas" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+1800 vendidos</p>
                                </div>

                                <p>Encuentra nuestros productos en <a href='https://kamas.juntoz.com/catalogo?top=28&skip=0&orderBy=rating-desc&skipSponsored=0' title='Encuentra nuestros productos en Juntoz'>Juntoz</a></p>
                            </div>
                        </li>

                        <li className='profile'>
                            <img src="/assets/imagenes/paginas/pagina-principal/perfiles/mercado-libre.webp" alt="Mercado Libre" className='profile-card-banner' />
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ9HpdIkdK6Ya0D-So2aV7brjbmPmqPhR-cYQ&s" alt="" className='profile-card-logo' />

                            <div className='profile-card-content d-flex-column'>
                                <div className='profile-card-stars'>
                                    <div className='d-flex-center-center'>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                        <span className="material-icons">star</span>
                                    </div>

                                    <p className='profile-card-sales'>+1600 vendidos</p>
                                </div>

                                <p>Encuentra nuestros productos en <a href='https://www.mercadolibre.com.pe/tienda/kamas?item_id=MPE765326810&category_id=MPE68117&official_store_id=202764&client=recoview-selleritems&recos_listing=true#origin=vip&component=sellerData&typeSeller=official_store' title='Encuentra nuestros productos en Mercado Libre'>Mercado libre</a></p>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </section>
    )
}

export default Perfiles;
