...
function Filtros({ onCambiarPrecio }){
    ...
    const handleSelectOption = (nombreFiltro, valor) => {
        const newSearchParams = new URLSearchParams(location.search);
        const transformedValor = valor.replace(/\s+/g, '-');

        if(nombreFiltro === 'detalles-del-producto'){
            const detallesActuales = { ...detallesSeleccionados };

            if(detallesActuales[valor]){
                delete detallesActuales[valor];
            } else {
                detallesActuales[valor] = transformedValor;
            }

            newSearchParams.set('detalles-del-producto', JSON.stringify(detallesActuales));
        } else {
            if(newSearchParams.get(nombreFiltro) === transformedValor){
                newSearchParams.delete(nombreFiltro);
            } else {
                newSearchParams.set(nombreFiltro, transformedValor);
            }
        }
    ...
    const handleSelectDetalle = (detalleKey, valor) => {
        ...
        if (detallesActuales[detalleKey] === transformedValor) {
            delete detallesActuales[detalleKey];
        } else {
            detallesActuales[detalleKey] = transformedValor;
        }
    ...
    const isFiltroSeleccionado = (filtroKey) => {
        return searchParams.has(filtroKey);
    };
    ...
    const isDetalleSeleccionado = (detalleKey, valor) => {
        return detallesSeleccionados[detalleKey] === valor.replace(/\s+/g, '-');
    };
